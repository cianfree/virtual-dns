...
public enum JavaVersion {
    ...
    private static float toFloatVersion(final String value) {
        final float defaultReturnValue = -1;
        if (value.contains(".")) {
            final String[] toParse = value.split("\\.");
            if (toParse.length >= 2) {
                return ConvertUtil.toFloat(toParse[0] + '.' + toParse[1], defaultReturnValue);
            }
        } else {
            return ConvertUtil.toFloat(value, defaultReturnValue);
        }
